...
export const PaymentEmail: FC<PaymentEmailProps> = ({ tariffs = [] }) => {
    ...
    const selectedTariff = Array.isArray(tariffs)
        ? tariffs.find((t) => t.id === selectedOption)
        : undefined

    const queryClient = useQueryClient()

    const mutation = useMutation({
        mutationFn: async (payload: { email: string; tariffId: string; promocode?: string }) => {
            const { email, tariffId, promocode } = payload

            const selectedTariff = tariffs.find((t) => t.id === tariffId)
            if (!selectedTariff) throw new Error('Неверный тариф')
            ...
            if (selectedTariff.is_trial) {
                ...
                if (!response.ok) {
                    throw new Error('Ошибка при активации пробного периода')
                }
            ...
            if (!isPaymentResponse(res)) {
                throw new Error('Не удалось получить подтверждение оплаты')
            }
        ...
        onSuccess: (user) => {
            ...
            toastSuccess('Поздравляем, вы успешно приобрели подписку!', 'Ваш профиль был обновлен!')
        },

        onError: (error: any) => {
            toastError('Ошибка при активации подписки', error?.message || 'Попробуйте еще раз')
        },
    })
        ...
        toast.custom(() => (
            <div className="bg-[#262833] p-4 rounded-2xl">
                <div className="flex items-center gap-4">
                    <ChecksIcon size={24} color="#FFFFFF" weight="light" />
                    <div>
            ...
            <div className="flex flex-col gap-14 max-w-md mx-auto w-full">
                ...
                >
                    <span className="font-semibold text-[#262833] text-sm">
                        {!loading ? 'Оформить подписку' : 'Оформляем вашу подписку...'}
                    </span>
