...
import { CommonQuestionsProps, CreateChatsProps } from "./types/types.api";
...
export const commonQuestions = async (): Promise<CommonQuestionsProps> => {
  try {
    const res = await internalFetch("https://профижкх.рф/api/v1/chats/common-questions", {
      method: "GET",
      credentials: "include",
      cache: 'no-cache',
      next: { revalidate: 3 },
    ...
    if (!res.ok) {
      console.error('Ошибка при получении дефолтных вопросов:', res);
      throw new Error('Failed to fetch user');
    }

    return data;
  } catch (error) {
    console.error('Сетевая ошибка при дефолтных вопросов:', error);
    throw error;
  }
};